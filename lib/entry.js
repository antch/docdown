...
function Entry(entry, source, lang) {
  ...
  this.getSinceVersion = _.memoize(getSinceVersion);
...
/**
 * Extracts the entry's since value.
 *
 * @memberOf Entry
 * @returns {string} The entry's since value
 */
function getSinceVersion() {
    return hasTag(this.entry, 'since')
        ? getValue(this.entry, 'since')
        : '';
}
...
_.assign(Entry.prototype, {
  ...
  'getType': getType,
  'getHash': getHash,
  'getSinceVersion': getSinceVersion
});
